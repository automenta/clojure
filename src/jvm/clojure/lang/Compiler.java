...
//            System.out.println("Remaining: " + RT.keys(m));
    ...
    static class DefExpr implements Expr {
        ...
        private static boolean includesExplicitMetadata(MapExpr expr) {
...
//		gen.invokeStatic(REFLECTOR_TYPE, getStaticFieldMethod);
    ...
    static class StaticMethodExpr extends MethodExpr {
        ...
        public StaticMethodExpr(String source, int line, int column, Symbol tag, Class c, String methodName, IPersistentVector args) {
            ...
            if (numMethods > 1) {
                ArrayList<Class[]> params = new ArrayList(numMethods);
                ArrayList<Class> rets = new ArrayList(numMethods);
                for (Object method1 : methods) {
                    java.lang.reflect.Method m = (java.lang.reflect.Method) method1;
                    params.add(m.getParameterTypes());
                    rets.add(m.getReturnType());
...
//		System.out.println("Class: " + cname);
    ...
    static class InvokeExpr implements Expr {
        ...
        public void emitProto(C context, ObjExpr objx, GeneratorAdapter gen) {
            ...
            gen.getStatic(objx.objtype, ObjExpr.cachedClassName(siteIndex), CLASS_TYPE); //target,class,cached-class
            ...
            gen.dup(); //target, target
            gen.invokeStatic(UTIL_TYPE, Method.getMethod("Class classOf(Object)")); //target,class
            gen.putStatic(objx.objtype, ObjExpr.cachedClassName(siteIndex), CLASS_TYPE); //target
...
//								}
                ...
                gen.dup();
                IPersistentVector fields = (IPersistentVector) Reflector.invokeStaticMethod(value.getClass(), "getBasis");
...
//		gen.getStatic(fntype, munge(k.sym.toString()), KEYWORD_TYPE);
        ...
        static String constantName(int id) {
        ...
        static String cachedClassName(int n) {
        ...
        try {
            ...
            for (int i = 0; i < objx.constants.count(); i++) {
                if (objx.usedConstants.contains(i))
                    cv.visitField(ACC_PUBLIC + ACC_FINAL + ACC_STATIC, ObjExpr.constantName(i), objx.constantType(i).getDescriptor(),
                            null, null);
            }
                ...
                try {
                    ...
                    for (int i = n * INITS_PER; i < objx.constants.count() && i < (n + 1) * INITS_PER; i++) {
                        if (objx.usedConstants.contains(i)) {
                            ...
                            clinitgen.putStatic(objx.objtype, ObjExpr.constantName(i), objx.constantType(i));
                        }
                    }
                } finally {
                    Var.popThreadBindings();
...
//		ret.volatiles = PersistentHashSet.create(RT.seq(RT.get(ret.optionsMap, volatileKey)));
    ...
    public static class NewInstanceMethod extends ObjMethod {
        ...
        static final Symbol dummyThis = Symbol.intern(null, "dummy_this_dlskjsdfower");
...
//				throw new IllegalArgumentException("Can't override/overload method: " + name.name);
        ...
        public void emit(ObjExpr obj, ClassVisitor cv) {
            Method m = new Method(name, retType, argTypes);
